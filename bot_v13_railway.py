...
class RiotAPIClient:
    ...
    async def get_live_matches(self) -> List[Dict]:
        ...
        # APENAS endpoints oficiais da Riot - sem simulação
        riot_endpoints = [
        ...
        for endpoint in riot_endpoints:
            try:
                # Headers padrão para evitar detecção
                ...
                async with aiohttp.ClientSession(timeout=self.timeout) as session:
                    async with session.get(endpoint, headers=current_headers) as response:
                        if response.status == 200:
                                ...
                                for match in matches:
                                    ...
                                    if len(teams) >= 2:
                                        ...
                                        if match_id not in seen_matches:
                                            ...
                                            # Log específico para partidas reais
                            ...
                            logger.debug(f"⚠️ Endpoint {endpoint}: status {response.status} - API bloqueada")
                                
            except Exception as e:
                logger.debug(f"❌ Erro no endpoint {endpoint}: {e}")
                continue
                    
        # Se não encontrou partidas reais, retornar lista vazia (SEM SIMULAÇÃO)
        if not all_matches:
