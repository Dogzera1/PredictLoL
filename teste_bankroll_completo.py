...
def test_bankroll_manager():
    ...
    print(f"   💰 Bankroll: R${manager.settings.current_bankroll:.2f}")
    ...
    if bet_result_2['success']:
        ...
        if resolve_2['success']:
            print(f"   💔 Aposta 2: {resolve_2['result']} - Prejuízo: R${resolve_2['profit']:.2f}")
        else:
            print(f"   ❌ Erro ao resolver aposta 2: {resolve_2['error']}")
    ...
    for amount, description in risk_scenarios:
        ...
        print(f"     • R${amount:.2f} ({percentage:.1f}%): {risk_level} - {description}")
