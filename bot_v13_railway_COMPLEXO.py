...
def main():
    ...
    try:
        ...
        # Inicializar bot
        # bot = LoLBotV3UltraAdvanced()  # TEMPORARIAMENTE DESABILITADO PARA TESTE
        ...
        if TELEGRAM_VERSION == "v20+":
            ...
            if is_railway:
                ...
                # LOGS CRÍTICOS ANTES DO app.run
                logger.info("🔷 DEBUG: Preparando para iniciar app.run...")
                logger.info(f"🔷 DEBUG: Host=0.0.0.0, Port={PORT}, Debug=False, Reloader=False, Threaded=True")
                logger.info("🔷 DEBUG: Chamando app.run agora...")
                
                try:
                    app.run(host='0.0.0.0', port=PORT, debug=False, use_reloader=False, threaded=True)
                    logger.info("🔷 DEBUG: app.run retornou (isso não deveria acontecer)")
                except Exception as app_run_error:
                    logger.error(f"🔷 DEBUG: ERRO NO APP.RUN: {app_run_error}")
                    import traceback
                    logger.error(f"🔷 DEBUG: TRACEBACK APP.RUN: {traceback.format_exc()}")
                    raise
